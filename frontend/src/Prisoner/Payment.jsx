...
const Payment = (props) => {
  ...
  function createEvent(id, dateTime, lawyerEmail) {
    var virtualEvent = {
      id: id,
      summary: "Appointment",
      location: "Virtual",
      description: "Lawyer-Prisoner appointment",
      start: {
        dateTime: dateTime,
  ...
  const { dateId, lawyer, slotId } = props.location.data;

  const bookSlot = async () => {
    const { data } = await Axios.post(
      `${process.env.REACT_APP_SERVER_URL}/lawyers/book-slot/`,
      {
        googleId: localStorage.getItem("googleId"),
        prisonerName: JSON.parse(localStorage.getItem("user")).name,
        slotId: slotId,
        dateId: dateId,
        lawyerId: lawyer._id,
      }
    ...
    if (data.lawyerEmail) {
      createEvent(data._id, data.date + "T" + data.slotTime, data.lawyerEmail);
    }
  };

  useEffect(() => {
    setFinalBalnce(1.18 * lawyer.feesPerSession);
  }, []);

  const makePayment = async (token) => {
    const { data } = await Axios.post(
      `${process.env.REACT_APP_SERVER_URL}/prisoners/payment`,
      {
        token,
        finalBalnce,
      }
                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td className="text-center">
                            <p>
                              <strong>{lawyer.feesPerSession}</strong>
                            </p>
                            <p>
                              <strong>{0.18 * lawyer.feesPerSession}</strong>
                            </p>
                          </td>
                        </tr>
                        <tr>
